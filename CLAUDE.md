...
- 印刷の「試算表（全科目）＋損益計算書3期比較（2ページ）」（`pageTrialFull`）は1資料＝2ページ。
  1ページ目は左に損益計算書・右に貸借対照表（取込んだ全科目・省略しない）、2ページ目はPLの3期比較。
  行数から文字サイズ段階（z1/z2/z3）を選び、それでも1枚に収まらない場合のみ可変高（`.flowsheet`）にして
  次の紙へ流す（科目を落とさないため）。比率の分母はCODESで取れないCSVでも小計行の名称から拾い直す
