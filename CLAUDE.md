...
- **紙面からはみ出した分は自動で縮めて必ず1ページに収める**（`theme.ts` の `AUTOFIT_JS`）。科目数は顧問先ごとに
  まったく違うので行数の決め打ちはしない。「組んでから測って、はみ出していたらその分だけ縮める」方式で、
  縮むのは本文だけ（フッターは絶対配置なので不動）。下限0.45まで縮むので**文字が切れることはない**。
  ページを新設するときも、この安全網があるので行数の見積もりは不要
