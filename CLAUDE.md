...
### 月次レポート
- **顧問先用アプリ（komon-app / `src/apps/keiei-report`）の月次レポート画面を丸ごと移植した**（2026-09）。
  移植先は `src/components/keiei/kr/`（画面）と `src/lib/keiei/kr/`（計算・データ）。
  移植元の型定義ごと持ち込み、`kr/state.ts`（アダプタ）と `kr/api.ts`（移植元と同じ形の窓口）で
  この総合管理アプリのデータに橋渡ししている。**計算コードには手を入れていない**ので
  顧問先が見る数字と必ず一致する。移植元を直したら差分を持ち込むだけでよい
- **報告月の反映はアダプタで行う**（`toKrState` が最新期の `lastFilledIndex` を選択月まで下げる）。
  ビューアの計算は全部 `lastFilledIndex` 基準なので、これで全画面が選択月ベースになる。
  各計算関数に monthIdx を配って回ると必ず漏れが出る（漏れてもエラーは出ず、
  「月を変えても数字が変わらない画面」になるだけなので気づけない）
- 画面は ダッシュボード／月次推移PL・BS／CF計算書（簡便法）／FCF・借入返済バランス／
  納税資金予測／損益分岐点／労働分配率／経営シミュレーション／顧問先の画面／AIに質問／
  AI質問ログ／データ取込(JSON)／元帳の取込／取引先の整理／使い方
- **AIに質問は「質問の文章だけ」をGeminiへ送る**（`kr/ask-ai.ts`）。金額・会社名・明細は送らない。
  返ってくるのは「どの集計で答えるか」の名前だけで、金額の計算はブラウザの中で行う。
  失敗してもキーワード判定に戻すだけで画面は壊さない
- **元帳CSVの取込口は1つ**。解析は `lib/keiei/ledger.ts`、保存は `lib/keiei/ledger-store.ts`（IndexedDB）で、
  税務チェックの会計監査と同じデータを見る。ビューア側は `kr/ledger/store.ts` が詰め替えるだけ
  （入口を2つ持つと同じCSVを2回取り込むことになり、監査側とずれる）
- **元帳の使い道**（取り込んであるときだけ有効になる機能）:
  ①月次推移PLの金額から明細へ降りる（`LedgerDrill.tsx`。当期実績の段のセルをクリック）
  ②取引先の整理（名寄せの手直し）③AIに質問の取引先系（合計・相手先別・月別）
  - **月別（partnerMonthly）は、合計を答えたときと同じ期間を暦月に割る**（年度の12ヶ月に
    決め打ちすると、「元帳の全期間」で答えた合計と月別の合計がずれる）。
    金額のあった月だけを「〇年〇月 〇円 〇件」で並べ、最後に合計行を置く
  - 合計の回答には「月別に見る」の提案（`Answer.followUp`）を出す。押すと同じ条件で
    別の集計を走らせるだけなので、AIへは問い合わせ直さず質問回数も消費しない
- **顧問先へ渡すJSONに元帳を同梱できる**（`export-data.ts` schema `keiei-monthly/2`）。
  書き出しは `ExportJsonDialog.tsx`、取込は `import-json.ts`＋`DataImport.tsx`。
  - 入れるのは**当期の1期だけ**（3期入れるとファイルが3倍。顧問先が見たいのはたいてい当期）
  - 入れる項目は受け取る側が使うものだけ（日付・科目・相手科目・摘要・借方・貸方）。
    消費税コード・税率は税務チェック用なので入れない
  - **名寄せの結果も同梱**する（顧問先の端末で計算し直させない・事務所と同じまとめ方になる）
  - **科目単位で除ける**。給料手当・役員報酬・事業主貸などは摘要に個人名が出るので既定で除外。
    元帳を入れると顧問先の画面に見えるものが増えるため、既定は「入れない」
  - **元帳部分だけ字下げなし**で書き出す（実測 245KB→136KB。試算表の部分は読める形のまま）
  - schema 1（試算表のみ）も引き続き読める。同梱された元帳が壊れていても試算表の取込は続ける
- **納税予測の前提は顧問先ごとに持つ**（`kr/TaxBasis.tsx`）。ここを間違えると予測が丸ごと外れる
  - **均等割は税率表をコードに焼き込まない**。都道府県分＋市町村分の2階建てで、県の上乗せ
    （茨城県の森林湖沼環境税など）・市町村の超過税率・資本金等の額・従業者数で変わり、
    毎年どこかの自治体が改正する。焼き込むと静かに間違えた金額を出し続けることになる。
    代わりに**事務所で1つ持つ自治体プリセット**（`lib/keiei/equalization-presets.ts`・
    RTDBの `keiei/_equalization`・顧問先削除では消さない）に金額を登録し、
    顧問先側は「自治体・資本金等の額・従業者数」を選ぶだけにする。該当区分が無ければ手入力に落ちる
  - **消費税は課税方式を持つ**（原則／簡易＋事業区分／免税）。**簡易課税は計算式そのものが違い**、
    仮払消費税は一切使わない（課税売上に係る消費税 ×（1−みなし仕入率））。
    簡易で区分も率も未指定のときは第3種70%を既定にする（null のままだと控除ゼロの過大な税額を黙って出す）
  - 仮受・仮払は残高そのままではなく **期首からの増加分**で見る（前期分の繰越が混ざらないように）。
    前期の通年データが無いときは残高のまま計算し、その旨を画面に出す
- **借入の返済予定を手入力で持てる**（`lib/keiei/loans.ts`＋`kr/LoanEditor.tsx`）。
  BSの残高の増減だけだと「過ぎたこと」しか分からず、これから何月にいくら返すのかが出せないため。
  - 入力は借入1本につき **当初額・実行日・回数・利率・返済方法（元金均等／元利均等／期限一括）＋据置**。
    毎月の元金と利息は計算で出す（返済予定表を1行ずつ写さない）。元利均等で実額が計算値と違うときだけ
    毎月返済額を直接入れられる。**端数は最終回で吸収して残高がぴったり0で終わる**
  - 保存は `keiei/{cid}/loans`（顧問先削除で一緒に消える）＋ localStorage `keiei-loans-{cid}`
  - **FCF・借入返済バランス**に「今後12ヶ月の約定返済」（元金・利息・借入残高の予定の折れ線）を出す
  - **納税資金予測**に「返済と納税の資金繰り」を出す。決算の納付＝決算月＋2ヶ月、
    中間納付＝期首＋8ヶ月に置き、**いちばん資金が要る月**を名指しする
    （返済と納税を別々に見ていると、実際に困る月が分からない）
- **CF計算書には月初・月末の現預金残高を出す**。残高は足し算ではないので、
  合計欄は期首残高と報告月末の残高（`CfRowDef.balance`）。月次の増減グラフには
  月末残高を**右目盛りの折れ線**で重ねる（増減と残高は桁が違うので同じ目盛りに載せない）
- **質問と回答のExcel出力**（`kr/qa-excel.ts`）: AIに質問の画面（その場の会話・回答の根拠つき）と
  AI質問ログの画面（保存した記録）の2か所から出せる。体裁は月次推移のExcel（`kr/excel.ts`）と揃え、
  Noto Sans JP・全セル罫線・見出しは濃紺。回答しなかった質問は薄い赤・読み取れなかった質問は薄い橙・
  ★は薄い青で塗る。**xlsx には行の高さを自動で合わせる指定が無い**ので、折り返しを有効にしたうえで
  列幅と文字数（全角は2文字ぶん）から高さを計算して入れる（既定の高さのままだと長い回答が1行しか見えない）
  - ドリルダウンは **元帳の合計と月次推移の金額を必ず並べて出す**。決算整理の前後や元帳の
    一部未取込でずれることがあり、黙って元帳だけ見せると気づけないため
  - 試算表と元帳で科目名の表記が違うことがある（車輌費／車両費）ので、名前で当たらなければ
    **科目コードで引く**（`buildAccountIndex` / `ledgerAccountOf`。明細は数万件になるので対応表は1回だけ作る）
- **取引先の名寄せは推測なので、答えを見てから「この表記だけ」へ切り替えられる**
  （`PartnerScope.variant` ＋ AIに質問の回答に出る `kr-vchoice`）。
  切替の再計算はブラウザの中だけで、AIには問い合わせ直さない（質問回数も消費しない）
- **名寄せ（`clusterPartners`）は索引と打ち切りで速くしてある**。総当たりだと件数の二乗になり、
  ユニークな摘要が1万を超える顧問先で取込のたびに数十秒固まっていた（実測 12,000表記で1.6〜42秒）。
  ①完全一致はキーで直接引く ②前方一致は先頭3文字で引く ③あいまい一致は「頭文字が同じ・
  長さが一致しうる範囲・含まれる文字の指紋が近い」ものだけ距離を計算し、その距離計算も
  帯（±cap）だけ埋めて打ち切る。**採用する候補は従来どおり「いちばん先に作られたグループ」**なので
  結果は1件も変わらない（新旧の突合で確認済み）。実測 12,000表記 1,634ms→356ms、24,000表記 1.1秒
- **取引先の整理は「確認が要るものだけ」を開いて見せる**（`Partners.tsx`）。
  まとまった理由（`MatchKind`）で3つに分け、**あいまい一致を含むものだけ★要確認**にする。
  別物が混ざるのはこの経路からしか出ない（「お茶代」と「お花代」）。並びは件数ではなく金額順。
  「これは別」で表記を1つ外せ、「このまとまりでOK」で `aliases.reviewed` に表記の指紋を残す。
  **次の取込で表記が増えたときだけ★へ戻る**ので、毎月ぜんぶ見直さなくてよい
- 印刷は **B4横カラー**（`kr/print.css` + `KrPrint.tsx`）。画面を選んで印刷／一括印刷ができる。
  用紙は `@page { size: 364mm 257mm }` と**実寸指定**（`size: B4` は ISO B4=250×353mm と解釈されうる）。
  `print-color-adjust: exact` が無いと表の網掛けだけ白く抜ける。
  **1画面＝1ページにはしない**（表とグラフの多い画面は0.2倍まで縮んで読めなくなるため、
  画面ごとに改ページして中身は必要なページ数だけ流す）。横幅が超える画面だけ縮小する。
  プレビューは body 直下へポータルで出す（`position:fixed` のままだと Chrome は1ページ目しか描かない）
- 旧機能（新報告書A4横9ページ・概要・試算表3期比較・原価経費明細・損益分岐点/CF・経営課題・
  予算予実・資金繰り・元帳分析・案件台帳）は 2026-09 に**削除した**。復活させず、移植した画面で対応する
- **`lib/keiei/` の以下は税務チェックが使うので消さないこと**:
  `audit.ts`（会計監査本体）・`ledger.ts`／`ledger-store.ts`（元帳）・`parse.ts`（CSVデコード）・
  `types.ts`・`store.ts`（loadYears）・`analysis.ts`／`calc.ts`／`tax.ts`（上記が依存）
