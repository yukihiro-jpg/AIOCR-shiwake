    ...
    return (
      <li key={folder.id}>
        ...
        >
          {children.length > 0 ? (
            <button
              ...
              className="w-4 h-5 shrink-0 text-gray-400 text-[10px] leading-none"
          ...
          <button
            ...
            className={`flex-1 min-w-0 flex items-center gap-1.5 py-1 pr-2 text-left text-[13px] ${selected ? 'font-semibold text-blue-700' : 'text-gray-700'}`}
          >
        ...
        {isOpen && children.length > 0 && (
          <ul>{children.map((c) => renderFolder(root, c, depth + 1))}</ul>
        )}
      </li>
    )
  }
        ...
        return (
          <li key={root.key}>
            <button
              ...
              className={`w-full flex items-center gap-1.5 px-1.5 py-1.5 rounded-md text-left text-[13px] ${rootSelected ? 'bg-blue-50 font-semibold text-blue-700' : 'text-gray-800 hover:bg-gray-50 font-medium'}`}
            >
            ...
            {top.length > 0 && <ul>{top.map((f) => renderFolder(root, f, 1))}</ul>}
          </li>
        )
      })}
    </ul>
