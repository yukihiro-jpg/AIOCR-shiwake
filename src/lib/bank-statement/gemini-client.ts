...
// 1リクエストの上限時間（ミリ秒）。通信ハング時に await が無限待ちになり、
// 画面の解析中表示・経過秒カウンターが止まらなくなるのを防ぐ（超過で自動abort→リトライ/終了）。
const REQUEST_TIMEOUT_MS = 120000
function gm(params: Parameters<GoogleGenerativeAI['getGenerativeModel']>[0]) {
  return genAI().getGenerativeModel(params, { timeout: REQUEST_TIMEOUT_MS })
}
...
page フィールドには0始まりのページ番号を入れてください（${startPage}～${endPage! - 1}）。`
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      const isRetriable = /429|503|504|timeout|abort|ECONN|fetch failed/i.test(msg)
...
// ============================================================
// 取引行の位置特定（参照ハイライト用）
// 抽出済みの取引リストを渡し、画像上の「どこにあるか」だけを返させる。
// 本体OCRとは独立した呼び出しなので、失敗しても取引データには影響しない。
// ============================================================

export interface LocatedRegions {
  rows: { index: number; box: [number, number, number, number] }[] // [ymin,xmin,ymax,xmax] 0-1000
  tables: {
    xRange: [number, number] // 表の水平範囲 0-1000（見開きスキャンでは左右2表）
    columns: Partial<Record<'date' | 'description' | 'withdrawal' | 'deposit' | 'amount' | 'balance', [number, number]>>
  }[]
}

const LOCATE_SCHEMA: ResponseSchema = {
  type: SchemaType.OBJECT,
  properties: {
    rows: {
      type: SchemaType.ARRAY,
      items: {
        type: SchemaType.OBJECT,
        properties: {
          index: { type: SchemaType.INTEGER, description: '取引リストのindex' },
          box: { type: SchemaType.ARRAY, items: { type: SchemaType.INTEGER }, description: '[ymin,xmin,ymax,xmax] 0-1000' },
        },
        required: ['index', 'box'],
      },
    },
    tables: {
      ...
      items: {
        type: SchemaType.OBJECT,
        properties: {
          xRange: { type: SchemaType.ARRAY, items: { type: SchemaType.INTEGER }, description: '表の水平範囲 [xmin,xmax] 0-1000' },
          date: { type: SchemaType.ARRAY, items: { type: SchemaType.INTEGER }, nullable: true, description: '日付列の [xmin,xmax] 0-1000' },
          description: { type: SchemaType.ARRAY, items: { type: SchemaType.INTEGER }, nullable: true, description: '摘要列の [xmin,xmax]' },
          withdrawal: { type: SchemaType.ARRAY, items: { type: SchemaType.INTEGER }, nullable: true, description: '出金列の [xmin,xmax]' },
          deposit: { type: SchemaType.ARRAY, items: { type: SchemaType.INTEGER }, nullable: true, description: '入金列の [xmin,xmax]' },
          amount: { type: SchemaType.ARRAY, items: { type: SchemaType.INTEGER }, nullable: true, description: '金額が1列の場合の [xmin,xmax]' },
          balance: { type: SchemaType.ARRAY, items: { type: SchemaType.INTEGER }, nullable: true, description: '残高列の [xmin,xmax]' },
        },
        required: ['xRange'],
      },
    },
  },
  required: ['rows', 'tables'],
}

/** 画像上で各取引行が占める矩形と、列の水平範囲を特定する（読み取りはしない） */
export async function locateTransactionRegions(
  image: string, // data URL（縮小済み推奨）
  txs: { date: string; description: string; deposit: number | null; withdrawal: number | null; balance: number }[],
  geminiModel?: string,
): Promise<LocatedRegions | null> {
  if (!txs.length) return null
  const list = txs.map((t, i) =>
    `${i}: ${t.date} | ${String(t.description || '').slice(0, 40)} | 入金:${t.deposit ?? '-'} | 出金:${t.withdrawal ?? '-'} | 残高:${t.balance}`
  ).join('\n')
  const prompt = `この画像は日本の銀行取引明細・通帳・出納帳の1ページです。
以下は既にこのページから読み取り済みの取引リストです（読み取り直す必要はありません）。

${list}

あなたの仕事は「位置の特定」だけです：
1. rows: 各取引(index)について、その取引行1行が画像上で占める矩形 box を返す。
   box は [ymin, xmin, ymax, xmax]、画像の高さ・幅に対する 0〜1000 の整数。
   行の上端〜下端をできるだけ正確に。摘要が2行に折り返す場合は両方を含む。
2. tables: 表の水平範囲 xRange と、各列（date/description/withdrawal/deposit/balance、
   入出金が1列しかない場合は amount）の水平範囲 [xmin,xmax] を返す。
   見開きスキャン等で独立した表が左右に2つある場合は tables を2要素にする。

全取引について rows を返してください。JSONのみを返してください。`
  try {
    const model = gm({
      model: resolveModel(geminiModel),
      generationConfig: {
        temperature: 0,
    ...
    const result = await model.generateContent([prompt, ...imagesToParts([image])])
    const text = result.response.text()
    const jsonMatch = text.match(/\{[\s\S]*\}/)
    if (!jsonMatch) return null
    const parsed = JSON.parse(jsonMatch[0]) as {
      rows?: { index?: number; box?: number[] }[]
      tables?: ({ xRange?: number[] } & Partial<Record<'date' | 'description' | 'withdrawal' | 'deposit' | 'amount' | 'balance', number[] | null>>)[]
    }
    const rows = (parsed.rows || [])
      .filter((r) => typeof r.index === 'number' && Array.isArray(r.box) && r.box.length === 4 && r.box.every((v) => typeof v === 'number'))
      .map((r) => ({ index: r.index as number, box: r.box as [number, number, number, number] }))
    const pair = (v: number[] | null | undefined): [number, number] | undefined =>
      Array.isArray(v) && v.length === 2 && v.every((n) => typeof n === 'number') ? [v[0], v[1]] : undefined
    const tables = (parsed.tables || [])
      .map((t) => {
        const xr = pair(t.xRange) || ([0, 1000] as [number, number])
        const columns: LocatedRegions['tables'][number]['columns'] = {}
        for (const key of ['date', 'description', 'withdrawal', 'deposit', 'amount', 'balance'] as const) {
          const p = pair(t[key])
          if (p) columns[key] = p
        }
        return { xRange: xr, columns }
      })
    if (!rows.length) return null
    return { rows, tables: tables.length ? tables : [{ xRange: [0, 1000], columns: {} }] }
  } catch (e) {
    console.log('locateTransactionRegions failed:', e)
    return null
  }
}
...
- orientation: 画像の向き。文字が正しく読める正位置にするために「画像を時計回りに回転させるべき角度」を 0/90/180/270 のいずれかで返す（既に正位置なら 0。上下逆さまなら 180。左に90度倒れて文字の上が画像の左を向いているなら 90）

出力は必ず以下のJSON形式のみ：
{"receipts": [{"receiptIndex": 0, "storeName": "スーパー", "receiptDate": "2025-03-15", "mainContent": "食料品・日用品", "invoiceNumber": "T1234567890123", "taxLines": [{"taxRate": "10%", "netAmount": 1000, "taxAmount": 100, "totalAmount": 1100}, {"taxRate": "8%", "netAmount": 2000, "taxAmount": 160, "totalAmount": 2160}], "pageIndex": 0, "orientation": 0}]}
...
const PROMPT_INVOICE_SALES = `この画像は売上請求書（当社＝発行者が取引先に発行した請求書）です（1つのPDFに複数の請求書が含まれる場合があります）。
各請求書について以下の情報をJSON形式で抽出してください。

【発行者と請求先の見分け方（最重要）】
- 請求書には「発行者（請求する側）」と「請求先（支払う側＝宛名）」の2つの名前が載っています
- 宛名（請求先）は通常、用紙の左上にあり「御中」「様」が付きます
- 発行者は通常、右上または下部にあり、住所・電話番号・登録番号（T+13桁）・社印・振込先口座とセットで記載されます
- これは売上請求書なので、counterpartName には必ず【宛名側（御中・様が付く側）】の名前を入れてください
- 発行者側（登録番号や振込先口座が併記されている側）の名前を counterpartName に入れてはいけません（それは当社自身です）
...
- counterpartName: 請求先名称（宛名。左上の「御中」「様」が付く会社名・個人名。敬称は除いて名前のみ）
...
const PROMPT_INVOICE_PURCHASE = `この画像は仕入請求書（当社が取引先から受け取った請求書）です（1つのPDFに複数の請求書が含まれる場合があります）。
各請求書について以下の情報をJSON形式で抽出してください。
...
- 宛名（請求先）は通常、用紙の左上にあり「御中」「様」が付きます
- 発行者は通常、右上または下部にあり、住所・電話番号・登録番号（T+13桁）・社印・振込先口座とセットで記載されます
- これは受け取った仕入請求書なので、counterpartName には必ず【発行者側（登録番号・振込先口座が併記されている側）】の名前を入れてください
- 宛名側（御中・様が付く側）は当社自身なので counterpartName に入れてはいけません
- invoiceNumber（登録番号）も必ず発行者側のものを読み取ってください
...
各請求書のフィールド：
- invoiceIndex: 請求書番号（0始まり）
- counterpartName: 請求元名称（発行者の会社名。右上または下部の、住所・登録番号・振込先とセットの側）
...
export async function askFilesQuestion(
  ...
  const model = gm({ model: resolveModel(geminiModel) })
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const parts: any[] = []
  parts.push({
    text:
