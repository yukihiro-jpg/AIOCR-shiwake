...
- NX-PRO の担当者ログインID とパスワード
- プリンタ（印刷する場合）

**パスワード・暗証番号はアプリの起動時に入力します。ファイルにも設定にも保存しません。**
NX-PRO の担当者パスワードとカードの暗証番号は別物なので、起動時に別々に尋ねます
（`config/settings.yaml` の `secrets`）。どちらも自動での入れ直しは行いません。
...
同梱のひな形には、NX-PRO の起動 →「担当者ログイン」→ アプリケーション一覧から
「電子申告」を起動、までの手順が入っています。事務所で直すのは次の3か所です。

```yaml
vars:
  login_id: '02'                 # 担当者ログインID
  login_user_name: '日下部 幸広'  # そのIDの担当者名（取り違え防止の照合に使う）
  start_app_name: '電子申告'      # 起動するアプリケーション
```

パスワードはここには書きません。`type_secret` が `settings.yaml` の
`secrets` で定義した `pw_nxpro` を参照します。

ログインIDを入れると担当者名が自動表示されるので、**パスワードを入れる前に
その名前が想定と一致するか確かめて**います。別の担当者としてログインすると、
その担当者の権限と履歴で申告することになるためです。

入力欄には表示名が付かないため、ひな形では並び順（`index: 0/1/2`）で指定しています。
`nxrpa inspect --title 担当者ログイン` で `auto_id` を採取して置き換えてください。
並び順のままだと、画面に欄が増えたときに静かにずれます。

### 4-5. 画面が変わったことを必ず確かめる
...
1. 起動時に**NX-PRO の担当者パスワード**と**カードの暗証番号**を順に求められます
2. 顧問先を選びます（複数選択できます）
3. 実行モードを選びます
